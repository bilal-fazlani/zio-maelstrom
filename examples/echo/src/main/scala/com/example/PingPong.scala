...
object PingPong extends ZIOAppDefault:
  ...
  val ping = NodeId("c4").ask[Pong](Ping(MessageId(6)), 9.seconds)
    .flatMap(_ => logInfo(s"PONG RECEIVED")).tapError(err => logError(s"ERROR: $err"))
    .catchAll(_ => ZIO.unit).flatMap(_ => exit(ExitCode.success))
