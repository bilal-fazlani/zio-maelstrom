...
/** Settings for the node
  ...
  * @param logLevel
  *   You can log messages using logInfo or logError. Output logs can be filtered by setting this value to either Disabled, Info or Error. Default is
  *   Info
  * @param logFormat
  *   You can choose between colored or plain logs. Default is colored
  * @param concurrency
  *   Concurrency level for processing messages. Default is 1024. This means 1024 request messages (receive api) + 1024 response messages (ask api) =
  *   2048 messages can be processed in parallel.
  */
case class Settings(
