...
// format: off
enum StandardErrorCode(code: Int, definite: Boolean) extends ErrorCode(code, definite):
  case Timeout extends StandardErrorCode(0, false) // Indicates that the requested operation could not be completed within a timeout.
  case NodeNotFound extends StandardErrorCode(1, true) // Thrown when a client sends an RPC request to a node which does not exist.
  case NotSupported extends StandardErrorCode(10, true) // Use this error to indicate that a requested operation is not supported by the current implementation. Helpful for stubbing out APIs during development.
  case TemporarilyUnavailable extends StandardErrorCode(11, true) // Indicates that the operation definitely cannot be performed at this time--perhaps because the server is in a read-only state, has not yet been initialized, believes its peers to be down, and so on. Do not use this error for indeterminate cases, when the operation may actually have taken place.
  case MalformedRequest extends StandardErrorCode(12, true) // The client's request did not conform to the server's expectations, and could not possibly have been processed.
  case Crash extends StandardErrorCode(13, false) // Indicates that some kind of general, indefinite error occurred. Use this as a catch-all for errors you can't otherwise categorize, or as a starting point for your error handler: it's safe to return internal-error for every problem by default, then add special cases for more specific errors later.
  case Abort extends StandardErrorCode(14, true) // Indicates that some kind of general, definite error occurred. Use this as a catch-all for errors you can't otherwise categorize, when you specifically know that the requested operation has not taken place. For instance, you might encounter an indefinite failure during the prepare phase of a transaction: since you haven't started the commit process yet, the transaction can't have taken place. It's therefore safe to return a definite abort to the client.
  case KeyDoesNotExist extends StandardErrorCode(20, true) // The client requested an operation on a key which does not exist (assuming the operation should not automatically create missing keys).
  case KeyAlreadyExists extends StandardErrorCode(21, true) // The client requested the creation of a key which already exists, and the server will not overwrite it.
  ...
  case TxnConflict extends StandardErrorCode(30, true) // The requested transaction has been aborted because of a conflict with another transaction. Servers need not return this error on every conflict: they may choose to retry automatically instead.
// format: on
